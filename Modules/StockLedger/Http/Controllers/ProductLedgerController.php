...
class ProductLedgerController extends BaseController
{
    public function index(){
        if(permission('product-ledger-access')){
            ...
            return view('stockledger::productLedger.index',$data);
        }else{
    ...
    public function productLedgerData(Request $request){
        ...
        $sales         = DB::table('products as p')
                        ...
                        ->join('sale_products as sp','sp.product_id','=','p.id')
                        ->join('sales as s','sp.sale_id','=','s.id')
                        ->join('customers as pa','s.customer_id','=','pa.id')
                        ->where([['s.status','=',2]])
                        ...
                        ->when(request('customer_id'),function($q3){
                            $q3->where('s.customer_id','=',request('customer_id'));
                        })
