            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar">
                        <a href="{{ route('dealer.panel.sale.add') }}" class="btn btn-warning btn-sm font-weight-bolder"><i class="fas fa-arrow-left"></i> Back</a>
                    </div>
                </div>
            </div>
                        ...
                        <form action="" id="sale_store_form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="form-group col-md-3 required">
                                    ...
                                    <input type="text" class="form-control" value="{{  Auth::guard('dealer')->user()->name }}" readonly  />
                                        ...
                                        @if (!$sale->sale_products->isEmpty())
                                            @foreach ($sale->sale_products as $key => $sale_product)
                                                <tr>
                                                    ...
                                                    <td><input type="text" class="form-control free_qty text-center" name="products[{{ $key + 1 }}][free_qty]" id="products_free_qty_{{ $key + 1 }}" value="{{ number_format($sale_product->pivot->free_qty,2,'.','') }}" data-row="{{ $key + 1 }}" readonly></td>
                                                    <td><input type="text" class="fcs text-right form-control net_unit_price" name="products[{{ $key + 1 }}][net_unit_price]" id="products_net_unit_price_{{ $key + 1 }}" value="{{ $product_price }}" data-row="{{ $key + 1 }}" readonly></td>
                                ...
                                <div class="form-group col-md-12 text-center pt-5">
                                    <a href="{{ url('dealer.panel.sale.add') }}" class="btn btn-danger btn-sm mr-3"><i class="far fa-window-close"></i> Cancel</a>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            function product_row_add(count){
                ...
                cols += `<td><input type="text" class="fcs form-control free_qty text-center" name="products[${count}][free_qty]" id="products_free_qty_${count}" value="0" data-row="${count}" readonly></td>`;
        ...
        function update_data(){
            ...
            if (rownumber < 0) {
                ...
                let url = "{{route('dealer.panel.sale.update')}}";
                $.ajax({
                    url: url,
                    type: "POST",
                    data: formData,
                    ...
                    success: function (data) {
                        ...
                        if (data.status == false) {
                            ...
                            if (data.status == 'success') {
                                window.location.replace("{{ route('dealer.panel.sale.index') }}");
