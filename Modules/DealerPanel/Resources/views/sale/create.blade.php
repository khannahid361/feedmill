                        ...
                        <form action="" id="sale_store_form" method="post" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <select class="form-control" id="dealer_id" disabled>
                                        @foreach ($dealer as $value)
                                            <option value="{{ $value->id }}" @if($value->id == Auth::guard('dealer')->user()->id) selected="selected" @endif>{{ $value->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-md-12 text-center pt-5">
                                    <button type="button" class="btn btn-danger btn-sm mr-3" onclick="window.location.replace('{{ route("dealer.panel.sale.add") }}');"><i class="fas fa-sync-alt"></i> Reset</button>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#dealer_id').on('change',function(){
                var id = $(this).val();
                $.get('{{ url("dealer/previous-balance") }}/'+id,function(data){
        ...
        function account_list(payment_method) {
            $.ajax({
                ...
                type   : "POST",
                data   : { payment_method: payment_method,_token: "{{csrf_token()}}"},
                success: function (data) {
        ...
        function store_data(){
            ...
            if (rownumber < 0) {
                ...
                let url = "{{route('dealer.panel.sale.store')}}";
                $.ajax({
                    url: url,
                    type: "POST",
                    data: formData,
