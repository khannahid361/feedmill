...
class SalesmenWiseSalesReport extends BaseModel{
    ...
    private function get_datatable_query(){
        ...
        $query = DB::table('salesmen as sm')
                ->selectRaw("SUM(s.grand_total) as total_amount, SUM(s.item) as total_item,SUM(s.total_qty) as total_qty,sm.name, sm.phone")
        ...
        if (!empty($this->_start_date) && !empty($this->_end_date)) {
            $query->whereDate('s.sale_date', '>=',$this->_start_date)->whereDate('s.sale_date', '<=',$this->_end_date);
        }
