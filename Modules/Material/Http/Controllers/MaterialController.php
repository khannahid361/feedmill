...
class MaterialController extends BaseController
{
                    ...
                    if ($request->update_id) {
                        ...
                        if ($has_opening_stock == 1) {
                            if (!empty($material_old_data->opening_stock_qty)) {
                                ...
                                $material_old_data->cost = $request->opening_cost;
    ...
    public function getAvailableBagQty(Request $request)
    {
        if ($request->ajax()) {
            $bagMaterials = WarehouseMaterial::with('material')
                ->whereHas('material', function ($query) {
                    $query->where('category_id', 14);
                })
                ->where([['warehouse_id', 1], ['material_id', $request->material_id]])->first();
