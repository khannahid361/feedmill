...
class Transfer extends BaseModel
{
    ...
    public function products()
    {
        return $this->belongsToMany(Product::class, 'transfer_products', 'transfer_id', 'product_id', 'id', 'id')
            ->withPivot('id', 'transfer_qty', 'receive_qty', 'damage_qty', 'unit_id', 'net_unit_cost', 'total', 'product_condition', 'material_id')
            ->withTimestamps();
    }
    ...
    private function get_datatable_query()
    {
        ...
        if (permission('transfer-inventory-bulk-delete')) {
            if (auth()->user()->warehouse_id) {
                ...
                $this->column_order = [
                    null, 't.id', 't.challan_no', 't.transfer_date', 't.from_warehouse_id', 't.to_warehouse_id', 't.item',
            ...
            if (auth()->user()->warehouse_id) {
                $this->column_order = [
                    't.id', 't.challan_no', 't.transfer_date', 't.item', 't.grand_total',
                    't.receive_date', 't.receive_status', null
                ];
            } else {
                $this->column_order = [
                    't.id', 't.challan_no', 't.transfer_date', 't.from_warehouse_id', 't.to_warehouse_id', 't.item',
                    't.grand_total', 't.transfer_status', 't.receiver_id',
                    't.receive_date', 't.receive_status', 't.created_by', null
                ];
            }
        ...
        if ($request->has('products')) {
            foreach ($request->products as $key => $value) {
                ...
                if (!isset($value['material_id'])) {
                    ...
                    $warehouse_material = WarehouseMaterial::where(['warehouse_id' => $request->to_warehouse_id, 'material_id'  => $value['material_id']])->first();
