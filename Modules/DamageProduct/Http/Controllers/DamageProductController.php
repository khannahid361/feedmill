...
class DamageProductController extends BaseController{
    ...
    public function product_search_with_id_for_damage(Request $request){
        if($request->ajax()) {
            ...
            ->where([['sp.sale_id',$request->sale_id], ['sp.product_id',(int)$request->data],])
            ...
            if($product) {
                ...
                $total_damage_qty = DB::table('damage_products')->where([['product_id',$product->product_id]])->sum('damage_qty');
