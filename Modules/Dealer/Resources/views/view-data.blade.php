        ...
        <div class="table-responsive col-9">
            <table class="table table-borderless">
                ...
                <tr>
                    ...
                    <td><b>:</b></td>
                    <td>{{ $dealer->created_by }}</td>
                    <td><b>Modified By</b></td>
                    <td><b>:</b></td>
                    <td>{{ $dealer->modified_by }}</td>
                </tr>
                <tr>

                    <td><b>Create Date</b></td>
                    <td><b>:</b></td>
                    <td>{{ $dealer->created_at ? date(config('settings.date_format'), strtotime($dealer->created_at)) : '' }}
                    </td>
                ...
                <tr>
                    <td><b>Status</b></td>
                    <td><b>:</b></td>
                    <td>{!! STATUS_LABEL[$dealer->status] !!}</td>
                </tr>
            </table>
        </div>
        <div class="col-md-3 text-center">
            @if ($dealer->avatar)
                <img src='{{ 'storage/' . DEALER_AVATAR_PATH . $dealer->avatar }}' alt='{{ $dealer->name }}'
                    style='width:150px;' />
            @else
                    ...
                    @foreach ($dealer->hasManyProducts as $key => $item)
                        <tr>
                            ...
                            <td class="text-center">
                                {{ $item->product->base_unit_price > 0 ? number_format(($item->commission_rate * 100) / $item->product->base_unit_price, 2, '.', ',') : 0 }}
                            </td>
                        </tr>
                    @endforeach
                @endif
