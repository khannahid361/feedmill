            ...
            <div class="card-header flex-wrap py-5">
                ...
                <div class="card-toolbar">
                    ...
                    @if (permission('salary-advance-add'))
                        <a href="javascript:void(0);" onclick="showFormModal('{{__('file.Add New Salary Advance')}}','{{__('file.Save')}}')" class="btn btn-primary btn-sm font-weight-bolder">
        ...
        <div class="card card-custom">
            <div class="card-header flex-wrap py-5">
                <form method="POST" id="form-filter" class="col-md-12 px-0">
                    <div class="row">
                        <x-form.selectbox labelName="{{__('Employee')}}" name="labour_id" id="labour_id" col="col-md-4" class="selectpicker">
                            @foreach ($labour as $row)
                                <option value="{{ $row->id }}">{{ $row->name }}--{{ $row->labour_id }}</option>
                            @endforeach
                        </x-form.selectbox>
                        <x-form.selectbox labelName="{{__('file.Advance Month')}}" name="advance_month" id="advance_month" col="col-md-4" class="selectpicker">
                            @foreach ($advance_pay as $row)
                                <option value="{{ $row->advance_month}}">{{ date('F-Y', strtotime($row->advance_month))}}</option>
                            @endforeach
                        </x-form.selectbox>
                        <div class="col-md-4">
                            ...
                            <table id="dataTable" class="table table-bordered table-hover">
                                <thead class="bg-primary">
                                    <tr>
                                        ...
                                        <th>{{__('file.Advance Month')}}</th>
                                        <th>{{__('file.Advance Amount')}}</th>
                                        <th>{{__('file.Advance Purpose')}}</th>
...
@include('hrm::labour.salary-advance.modal')
@endsection
@push('scripts')
...
<script>
    ...
    $(document).ready(function(){
        ...
        table = $('#dataTable').DataTable({
            ...
            "ajax": {
                "url": "{{route('labourSalaryAdvance.datatable.data')}}",
                "type": "POST",
                "data": function (data) {
        ...
        $(document).on('click', '#save-btn', function () {
            ...
            let url         = "{{route('labourSalaryAdvance.store.or.update')}}";
        ...
        $(document).on('click', '.edit_data', function () {
            ...
            if (id) {
                $.ajax({
                    url: "{{route('labourSalaryAdvance.edit')}}",
                    type: "POST",
