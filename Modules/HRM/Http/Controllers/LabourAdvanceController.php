...
use Modules\HRM\Entities\LabourAdvance;
use Modules\HRM\Http\Requests\LabourSalaryAdvanceFormRequest;

class LabourAdvanceController extends BaseController
{
    public function __construct(LabourAdvance $model)
    {
        $this->model = $model;
    }
    public function index(){
        if(permission('salary-advance-access')){
            $setTitle = __('file.Labour Salary Advance');
            $this->setPageData($setTitle, $setTitle, 'far fa-handshake', [['name' => $setTitle]]);
            $labour         = Labour::where('status',1)->get();
            $advance_pay    = LabourAdvance::select('id','advance_month')->groupBy('advance_month')->get();
            return view('hrm::labour.salary-advance.index',compact('labour','advance_pay'));
        }else{
        ...
        if($request->ajax()){
            if(permission('salary-advance-access')){
                ...
                foreach ($list as $value) {
                    ...
                    $row[]  = permission('salary-advance-edit') ? change_status($value->id,$value->status, $value->labour->name) : STATUS_LABEL[$value->status];
    ...
    public function store_or_update_data(LabourSalaryAdvanceFormRequest $request) {
        if($request->ajax()){
