...
class DealerSaleController extends BaseController
{
    ...
    public function changeStatus(Request $request)
    {
        if ($request->ajax() && permission('sale-status-change')) {
            ...
            try {
                ...
                if(!empty($monthlyTraget))
                {
                    $monthlyTraget->acheived_qty += $sale->total_qty;
                    $monthlyTraget->update();
                }
    ...
    private function dealerAccounts($dealerAccountId, $invoiceNo, $saleDate, $dealerName, $grandTotal, $totalTax, $paymentData)
    {
        ...
        $saleChartOfAccountTransaction             = collect([
            'chart_of_account_id' => $dealerAccountId, 'voucher_no' => $invoiceNo, 'voucher_type' => 'INVOICE', 'voucher_date' => $saleDate,
        ...
        $productSaleChartOfAccountTransaction      = collect([
            'chart_of_account_id' => $productSaleChartOfAccountId, 'voucher_no' => $invoiceNo, 'voucher_type' => 'INVOICE', 'voucher_date' => $saleDate,
        ...
        if ($totalTax > 0) {
            $taxChartOfAccountTransaction              = collect([
                'chart_of_account_id' => $taxChartOfAccountId, 'voucher_no' => $invoiceNo, 'voucher_type' => 'INVOICE', 'voucher_date' => $saleDate,
        ...
        if (!empty($paymentData['paid_amount'])) {
            ...
            if ($paymentData['payment_method'] == 1) {
                $payment                               = collect([
                    'chart_of_account_id' => $paymentData['account_id'], 'voucher_no' => $invoiceNo, 'voucher_type' => 'INVOICE', 'voucher_date' => $saleDate,
    ...
    private function warehouseAccounts($warehouseId, $invoiceNo, $saleDate, $cost)
    {
        ...
        $inventory = collect([
            'chart_of_account_id' => $inventoryId, 'warehouse_id' => $warehouseId, 'voucher_no' => $invoiceNo, 'voucher_type' => 'INVOICE', 'voucher_date' => $saleDate,
