        ...
        <div class="container-fluid">
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar"><a href="{{ route('dealer.sale') }}" class="btn btn-warning btn-sm font-weight-bolder"><i class="fas fa-arrow-left"></i>{{'Back'}}</a></div>
                </div>
            </div>
            <div class="card card-custom" style="padding-bottom: 100px !important;">
                <div class="card-body">
                    <form id="store_form" method="post" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" id="sale_id" name="dealer_sale_id" value="{{$details->id}}"/>
                        <div class="row">
                            <div class="form-group col-md-4 required">
                                <label for="customer">{{'Dealer'}}</label>
                                <input type="text" class="form-control" value="{{$details->dealer->name}}"/>
                                <input type="hidden" id="dealer_id" name="dealer_id" value="{{$details->dealer_id}}"/>
                            </div>
                            <div class="form-group col-md-4 required">
    ...
    <script type="text/javascript">
        ...
        $(document).on('change','#warehouse_id',function(){
            ...
            if(warehouseId != ''){
                ...
                $.ajax({
                    url     : "{{route('dealer.sale.product.delivery.available.list')}}",
        ...
        function storeData(){
            ...
            let url      = "{{route('dealer.sale.product.delivery.store')}}";
            let delivery = 0;
            $('.delivery').each(function() {
                if($(this).val() == ''){
                    delivery += + 0;
            ...
            if(delivery > 0){
                $.ajax({
                    ...
                    success     : function (data) {
                        ...
                        if (data.status == 'success') { window.location.replace("{{ route('dealer.sale') }}"); }
                    },
