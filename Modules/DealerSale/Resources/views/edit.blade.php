            ...
            <div class="card card-custom gutter-b">
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar">
                        <a href="{{ route('dealer.sale') }}" class="btn btn-warning btn-sm font-weight-bolder"><i
                                            ...
                                            @if (!$sale->sale_products->isEmpty())
                                                @foreach ($sale->sale_products as $key => $sale_product)
                                                    <tr>
                                                        ...
                                                        <td class="product-code_tx_{{ $key + 1 }} text-center"
                                                            id="products_code_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">{{ $sale_product->code }}</td>
                                                        <td class="unit-name_tx_{{ $key + 1 }} text-center"
                                                            id="products_unit_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">{{ $temp_unit_name[0] }}</td>
                                                        <td class="available-qty_tx_{{ $key + 1 }} text-center"
                                                            id="products_available_qty_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                            {{ $stock_qty - $sale_product->pivot->qty }}</td>
                                                        <td><input type="text" class="form-control qty text-center"
                                                                name="products[{{ $key + 1 }}][qty]"
                                                                id="products_qty_{{ $key + 1 }}"
                                                                value="{{ number_format($sale_product->pivot->qty, 2, '.', '') }}"
                                                                data-row="{{ $key + 1 }}"></td>
                                                        <td><input type="text" class="form-control free_qty text-center"
                                                                name="products[{{ $key + 1 }}][free_qty]"
                                                                id="products_free_qty_{{ $key + 1 }}"
                                                                value="{{ number_format($sale_product->pivot->free_qty, 2, '.', '') }}"
                                                                data-row="{{ $key + 1 }}"></td>
                                                        <td><input type="text"
                                                                class="fcs text-right form-control net_unit_price"
                                                                name="products[{{ $key + 1 }}][net_unit_price]"
                                                                id="products_net_unit_price_{{ $key + 1 }}"
                                                                value="{{ $product_price }}"
                                                                data-row="{{ $key + 1 }}"></td>
                                                        <td class="tax text-right" id="tax_tx_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                            {{ number_format((float) $sale_product->pivot->tax, 2, '.', '') }}
                                                        </td>
                                                        <td class="sub-total text-right"
                                                            id="sub_total_tx_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                            {{ number_format((float) $sale_product->pivot->total, 2, '.', '') }}
                                                        </td>
                                                        <input type="hidden" class="product-id_vl_{{ $key + 1 }}"
                                                            name="products[{{ $key + 1 }}][id]"
                                                            value="{{ $sale_product->pivot->product_id }}"
                                                            id="products_id_vl_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                        <input type="hidden" class="product-code_vl_{{ $key + 1 }}"
                                                            name="products[{{ $key + 1 }}][code]"
                                                            value="{{ $sale_product->code }}"
                                                            id="products_code_vl_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                        <input type="hidden" class="batch-no_vl_{{ $key + 1 }}"
                                                            name="products[{{ $key + 1 }}][batch_no]"
                                                            id="products_batch_no_{{ $key + 1 }}"
                                                            value="{{ $sale_product->pivot->batch_no }}"
                                                            data-row="{{ $key + 1 }}">
                                                        <input type="hidden"
                                                            class="stock-qty_vl_{{ $key + 1 }} form-control text-center"
                                                            name="products[{{ $key + 1 }}][stock_qty]"
                                                            value="{{ $stock_qty - $sale_product->pivot->qty }}"
                                                            id="products_stock_qty_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                        <input type="hidden"
                                                            class="free-stock-qty_vl_{{ $key + 1 }} form-control text-center"
                                                            name="products[{{ $key + 1 }}][free_stock_qty]"
                                                            value="{{ $sale_product->pivot->free_qty }}"
                                                            id="products_free_stock_qty_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                        <input type="hidden" class="sale-unit_vl_{{ $key + 1 }}"
                                                            name="products[{{ $key + 1 }}][unit]"
                                                            value="{{ $unit_name }}"
                                                            id="sale_unit_vl_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                        <input type="hidden"
                                                            class="sale-unit-operator_vl_{{ $key + 1 }}"
                                                            value="{{ $unit_operator }}"
                                                            id="sale_unit_operator_vl_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                        <input type="hidden"
                                                            class="sale-unit-operation-value_vl_{{ $key + 1 }}"
                                                            value="{{ $unit_operation_value }}"
                                                            id="sale_unit_operation_value_vl_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                        <input type="hidden" class="tax-rate"
                                                            name="products[{{ $key + 1 }}][tax_rate]"
                                                            value="{{ $sale_product->pivot->tax_rate }}"
                                                            id="tax_rate_vl_{{ $key + 1 }}"
                                                            data-row="{{ $key + 1 }}">
                                                        @if ($tax)
                                                            <input type="hidden" class="tax-name"
                                                                value="{{ $tax->name }}"
                                                                id="tax_name_vl_{{ $key + 1 }}"
                                                                data-row="{{ $key + 1 }}">
                                                        @else
                                            ...
                                            @if (!$taxes->isEmpty())
                                                @foreach ($taxes as $tax)
                                                    <option value="{{ $tax->rate }}"
                                                        {{ $sale->order_tax_rate == $tax->rate ? 'selected' : '' }}>
                                                        {{ $tax->name }}</option>
                                                @endforeach
                                            @endif
                                        </x-form.selectbox>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="form-group col-md-4 required">
                                            ...
                                            <input type="text" class="form-control" name="net_total" id="net_total"
                                                value="{{ $sale->grand_total + $sale->previous_due }}" readonly>
                                        </div>
                                        <div class="form-group col-md-4 required">
                                            <label for="paid_amount">Paid Amount</label>
                                            <input type="text" class="form-control" name="paid_amount"
                                                id="paid_amount" value="{{ $sale->paid_amount }}">
                                        </div>
                                        ...
                                        <x-form.selectbox labelName="Payment Method" name="payment_method"
                                            ...
                                            @foreach (SALE_PAYMENT_METHOD as $key => $value)
                                                <option value="{{ $key }}"
                                                    @if ($sale->payment_method) {{ $sale->payment_method == $key ? 'selected' : '' }} @endif>
                                                    {{ $value }}</option>
                                            @endforeach
                                        </x-form.selectbox>
                                        <x-form.selectbox labelName="Account" name="account_id" required="required"
                                            col="col-md-4" />
                                        <div
                                            class="form-group required col-md-4 @if ($sale->payment_method) {{ $sale->payment_method != 1 ? '' : 'd-none' }} @endif reference_no">
                                            <label for="reference_no">Reference No</label>
                                            <input type="text" class="form-control" name="reference_no"
                                                id="reference_no" value="{{ $sale->reference_no }}">
                                        </div>
                                    </div>
                                </div>
                                <div class="form-group col-md-12 text-center pt-5">
                                    <a href="{{ url('sale') }}" class="btn btn-danger btn-sm mr-3"><i
