...
use Modules\Account\Entities\DealerYearlyCommissionPayment;
use Modules\Account\Entities\Transaction;

class DealerYearlyCommissionPaymentController extends BaseController
{
    public function __construct(DealerYearlyCommissionPayment $model)
    {
        $this->model = $model;
    }
                ...
                foreach ($list as $value) {
                    if ($value->debit > 0) {
                        ...
                        if (permission('supplier-payment-details')) {
                            $action .= ' <a class="dropdown-item" href="' . route('dealer.yearly.commission.payment.show', ['id' => $value->id]) . '">' . self::ACTION_BUTTON['View'] . '</a>';
                        }
    ...
    public function show(int $id)
    {
        ...
        return view('account::yearly-commission-payment.print', compact('data'));
        // }else{
