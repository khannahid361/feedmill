...
class SalesMenPayment extends BaseModel {
    protected const VOUCHER_PREFIX = 'SPM';
    ...
    public function setSalesMenCOAID($salesmen_coa_id){
        $this->_salesmen_coa_id = $salesmen_coa_id;
    }
    private function getDataTableQuery(){
        ...
        $query = DB::table('transactions as t')
            ...
            ->leftJoin('salesmen as s','coa.salesmen_id','=','s.id')
            ->selectRaw("t.*,coa.id as coa_id,coa.salesmen_id,coa.name,s.name as salesmen_name")
        ...
        if (!empty($this->_salesmen_coa_id)) {
            $query->where('t.chart_of_account_id', $this->_salesmen_coa_id);
        }
    ...
    public function count_all(){
        $query =  DB::table('transactions as t')
            ->join('chart_of_accounts as coa','t.chart_of_account_id','=','coa.id')
            ->selectRaw("t.*,coa.id as coa_id,coa.salesmen_id,coa.name")
            ->where('t.voucher_type',self::VOUCHER_PREFIX)
            ->whereNotNull('coa.salesmen_id');
        return $query->get()->count();
    }
