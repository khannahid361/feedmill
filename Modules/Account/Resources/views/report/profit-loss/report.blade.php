...
function AssertCoa($name,$code,$general_ledger,$asset_accounts,$visited,$value,$start_date,$end_date,$check,$warehouse_id){
  if($value==1) {
  ?>
    <tr>
    ...
    if($check) {
        $oResultAmountPreF = DB::table('transactions as t')
                            ...
                            ->when($warehouse_id, function ($q) use ($warehouse_id) {
                                return $q->where('t.warehouse_id', $warehouse_id);
                            })
  ...
  for($i=0;$i<count($asset_accounts);$i++) {
      if (!$visited[$i] && $general_ledger == 2) {
        if ($name == $asset_accounts[$i]->parent_name) {
          $visited[$i]=true;
          AssertCoa($asset_accounts[$i]->name,$asset_accounts[$i]->code,$asset_accounts[$i]->general_ledger,$asset_accounts,$visited,$value+1,$start_date,$end_date,$check,$warehouse_id);
        }
      }
  }
}
            ...
            <div id="printArea">
                <div class="panel-body">
                  ...
                  <table class="table" width="100%" class="table_boxnew" cellpadding="5" cellspacing="0">
                    ...
                    AssertCoa("COA","0",2,$asset_accounts,$visited,0,$start_date,$end_date,0,$warehouse_id);
                    ...
                    AssertCoa("COA","0",2,$liability_accounts,$visited,0,$start_date,$end_date,1,$warehouse_id);
