                ...
                <div class="card-header flex-wrap py-5">
                    ...
                    <div class="card-toolbar">
                        @if (permission('salesmen-payment-access'))
                            <a href="{{ route('salesmen.payment') }}"  class="btn btn-warning btn-sm font-weight-bolder"><i class="fas fa-arrow-circle-left"></i> Back</a>
                        @endif
                    </div>
                </div>
            </div>
                    ...
                    <div id="kt_datatable_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
                        <form id="salesmen-payment-form" method="post">
                            ...
                            <div class="row">
                                ...
                                <x-form.selectbox labelName="Salesmen" name="salesmen_id"  onchange="dueAmount(this.value)" required="required"  col="col-md-4" class="selectpicker">
                                    @if (!$salesmen->isEmpty())
...
@push('scripts')
    ...
    <script>
        ...
        function store_data(){
            ...
            let url = "{{route('salesmen.payment.store')}}";
            $.ajax({
                url: url,
                type: "POST",
                data: formData,
                ...
                success: function (data) {
                    ...
                    if (data.status == false) {
                        ...
                        window.location.replace("{{ url('salesmen-payment') }}");
                    }
                },
                error: function (xhr, ajaxOption, thrownError) { console.log(thrownError + '\r\n' + xhr.statusText + '\r\n' + xhr.responseText); }
            });
