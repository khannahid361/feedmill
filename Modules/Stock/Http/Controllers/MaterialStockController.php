    ...
    public function getMaterialStockData(Request $request){
        if($request->ajax() && permission('material-stock-report-access')){
            ...
            foreach ($list as $value) {
                ...
                $row[] = $value->cost ? number_format($value->cost,4,'.',',') : ($value->opening_cost ? number_format($value->opening_cost,2,'.',','): 0);
                $row[]  = number_format($value->qty,4);
                // $row[]  = number_format(($value->cost * $value->qty),2);
                $row[] = $value->cost ? number_format($value->qty * $value->cost,4,'.',',') : ($value->opening_cost ? number_format($value->qty * $value->opening_cost,4,'.',','): 0);
                $data[] = $row;
            }
